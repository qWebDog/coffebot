...
async def cmd_start(msg: Message, state: FSMContext, bot: Bot):
    ...
    if menu_photo:
        await msg.answer_photo(photo=menu_photo, caption=text, reply_markup=kb, parse_mode="Markdown")
    else:
        await msg.answer(text=text, reply_markup=kb, parse_mode="Markdown")
...
async def back_main(call: CallbackQuery, bot: Bot, state: FSMContext):
    ...
    menu_photo = await db.get_setting("menu_photo")
    if menu_photo:
        await call.message.edit_media(media=InputMediaPhoto(media=menu_photo), caption="☕ Выберите категорию:", reply_markup=main_menu_kb(cats))
    else:
...
async def show_category(call: CallbackQuery, bot: Bot, state: FSMContext):
    ...
    if cat.get("photo_id"):
        await call.message.edit_media(media=InputMediaPhoto(media=cat["photo_id"]), caption=text, reply_markup=kb, parse_mode="Markdown")
    else:
...
async def select_item(call: CallbackQuery, state: FSMContext, bot: Bot):
    ...
    # ✅ ИСПРАВЛЕНО: edit_caption вместо edit_text (текущее сообщение - фото)
    try:
        await call.message.edit_caption(caption="📏 Выберите объем:", reply_markup=item_volumes_kb(vols))
    except TelegramBadRequest:
        # Фолбэк: если подпись не редактируется, удаляем фото и шлём текст
        await call.message.delete()
        await call.message.answer("📏 Выберите объем:", reply_markup=item_volumes_kb(vols))
...
async def checkout(call: CallbackQuery, state: FSMContext, bot: Bot):
    ...
    await call.message.edit_text(f"💳 Оформление заказа на {int(total)}₽\n(ЮKassa подключается здесь)", reply_markup=InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text="🔙 Назад", callback_data="view_cart")]]))
    await call.answer()
