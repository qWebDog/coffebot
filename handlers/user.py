...
class UserFSM(StatesGroup):
    ...
    viewing_cart = State()
    selecting_time = State()  # ✅ Состояние для выбора времени
...
async def pick_volume(call: CallbackQuery, state: FSMContext, bot: Bot):
    ...
    if not extras: return await finalize_add_to_cart(call, state, bot, [])
    photo = extras[0].get("photo_id")
    text, kb = "🥐 Выберите дополнительные опции:", extras_kb(extras, [])
    if photo: await call.message.edit_media(media=InputMediaPhoto(media=photo), caption=text, reply_markup=kb)
...
async def finalize_add_to_cart(call: CallbackQuery, state: FSMContext, bot: Bot, extra_ids: list[int]):
    ...
    extras, extras_total = [], 0.0
    for eid in extra_ids:
        e = next((ex for ex in await db.get_extras() if ex["id"] == eid), None)
        if e: extras.append({"id": e["id"], "name": e["name"], "price": e["price"]}); extras_total += e["price"]
    lines = await db.get_cart(call.from_user.id)
    lines.append({"item_id": item_id, "name": item_name, "vol": vol_name, "vol_id": vol_id, "qty": 1, "extras": extras, "line_total": base_price + extras_total})
...
async def checkout(call: CallbackQuery, state: FSMContext, bot: Bot):
    ...
    await state.update_data({"time_offset": 5})  # Старт отсчёта: +5 минут
    await call.message.edit_text("🕒 Выберите удобное время для получения:", reply_markup=time_selection_kb(5))
...
async def time_plus(call: CallbackQuery, state: FSMContext, bot: Bot):
    data = await state.get_data()
    offset = data.get("time_offset", 5) + 5
    await state.update_data({"time_offset": offset})
    await call.message.edit_reply_markup(reply_markup=time_selection_kb(offset))
    await call.answer()
...
async def time_minus(call: CallbackQuery, state: FSMContext, bot: Bot):
    ...
    offset = data.get("time_offset", 5) - 5
    if offset < 5:
        return await call.answer("⏳ Минимальное время: 5 минут от текущего", show_alert=True)
    await state.update_data({"time_offset": offset})
    await call.message.edit_reply_markup(reply_markup=time_selection_kb(offset))
    await call.answer()
...
async def time_confirm(call: CallbackQuery, state: FSMContext, bot: Bot):
    ...
    await call.message.edit_text(
        ...
        reply_markup=InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text="🔙 Назад к корзине", callback_data="checkout")]]),
        parse_mode="Markdown"
    )
    await call.answer()

@router.callback_query(F.data == "time_ignore")
async def time_ignore(call: CallbackQuery): await call.answer()
